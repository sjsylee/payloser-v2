...
import { Crown, Image as ImageIcon, UserMinus, UserPlus } from "lucide-react";
import type {
  ApiGroup,
  ApiGroupJoinRequest,
  ApiGroupMember,
            ...
            {canManage ? (
              ...
              >
                ...
                <div className="mt-2 grid grid-cols-[52px_minmax(0,1fr)_66px] gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={disabled || uploadingImage || !updateGroupInput}
            ...
            <div className="mt-3 rounded-[24px] bg-[#F4F0E8] p-3">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                {activeMembers.map((member) => {
                  const canRemoveMember = canManage && member.role !== "OWNER";
                  const confirmingRemove = memberRemoveConfirmId === member.id;
                  const removing = memberRemoveWorkingId === member.id;

                  return (
                    <div
                      key={member.id}
                      className={`min-w-0 rounded-[18px] p-2 ${
                        ...
                        confirmingRemove ? (
                          <div className="mt-2 grid grid-cols-2 gap-1">
                            <button
                              ...
                              disabled={
                                disabled || Boolean(memberRemoveWorkingId)
                              }
                              className="h-8 rounded-[13px] bg-white/70 text-[10px] font-black text-ink/50 disabled:opacity-40"
                            >
                              취소
                            </button>
                            <button
                              type="button"
                              onClick={() => void submitRemoveMember(member.id)}
                              disabled={disabled || removing}
                              className="h-8 rounded-[13px] bg-ink text-[10px] font-black text-white disabled:opacity-40"
                            >
                              {removing ? "처리중" : "내보내기"}
                            </button>
                          </div>
                          ...
                          <button
                            ...
                            className="mt-2 flex h-8 w-full items-center justify-center gap-1 rounded-[13px] bg-[#F4F0E8] text-[10px] font-black text-ink/48 disabled:opacity-40"
                          >
