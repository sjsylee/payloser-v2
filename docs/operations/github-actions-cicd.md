...
새 PR을 열거나 기존 PR 브랜치에 추가 커밋을 push하기 전에는 브랜치가 이미 `main`에 포함됐는지 먼저 확인합니다. 이미 merge된 PR 브랜치에 추가 커밋을 올리면 GitHub PR 화면에서는 닫힌 PR로 보이지만, 실제 변경은 새 PR로 전달되지 않을 수 있습니다.

Before opening a new PR or pushing more commits to an existing PR branch, check whether the branch is already included in `main`. Pushing more commits to a merged PR branch can leave the new work outside any open PR.
...
| 결과 / Result            | 의미 / Meaning                                    | 다음 행동 / Next Step                         |
| ------------------------ | ------------------------------------------------- | --------------------------------------------- |
| `cleanly ahead`          | 기준 브랜치 위에 새 커밋만 있는 상태              | 현재 브랜치로 PR 생성 또는 업데이트           |
| `already merged`         | 현재 브랜치 HEAD가 이미 기준 브랜치에 포함된 상태 | `origin/main`에서 새 브랜치 생성              |
| `diverged from the base` | 기준 브랜치와 현재 브랜치가 서로 다른 커밋을 가짐 | 새 브랜치 생성 후 필요한 커밋만 `cherry-pick` |
