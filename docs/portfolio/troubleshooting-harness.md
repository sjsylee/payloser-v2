...
`NEXT_PUBLIC_API_BASE_URL`, `NEXT_PUBLIC_KAKAO_JAVASCRIPT_KEY`를 GitHub Actions repository variable 또는 secret에서도 읽도록 Web CD를 변경했습니다. Vercel production environment pull은 project settings와 fallback 용도로 유지하되, prebuilt build에 필요한 public 값은 GitHub Actions env로 먼저 주입합니다.
